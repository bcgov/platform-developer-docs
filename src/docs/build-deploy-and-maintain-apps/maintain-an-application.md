...
The platform services team runs weekly [automation](../platform-architecture-reference/platform-automation.md) that scales down `deployments` and `deploymentconfigs` in non-production environments if they have not been updated in over a year, or if they're crashing continuously.
