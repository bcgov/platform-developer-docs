...
We assume you already know the best practices mentioned in [Build your application](/build-an-application/) if not, make sure to check it out before continuing with this part of the guide. 
...
This process works hand-in-hand with the build process, detailed in our [build an application](/build-an-application/) documentation . Following sections will dive deeper into each deployment stage.
...
OpenShift provides robust monitoring capabilities to ensure your application is running as expected. You can utilize services like [Sysdig](/sysdig-monitor-onboarding/) for monitoring the health and performance of your application, and [Uptime.com](https://uptime.com/) for availability monitoring.

- **Alerts**: Proactive alerting is crucial for maintaining system reliability. Alerts notify you of potential issues before they become critical problems. Sysdig provides features to create alerts based on certain conditions. Learn more about [setting up alert channels in Sysdig](/sysdig-monitor-create-alert-channels/)
